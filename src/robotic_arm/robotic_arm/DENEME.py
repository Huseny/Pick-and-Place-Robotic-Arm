...
def generate_launch_description():
    ...
    robot_description_path = LaunchConfiguration(
        ...
        default=get_package_share_directory("my_robot_description")
        + "/urdf/my_robot.urdf",
    )

    # Set the path to the camera URDF file
    camera_description_path = LaunchConfiguration(
        "camera_description_path",
        default=get_package_share_directory("my_robot_description")
        + "/urdf/my_camera.urdf.xacro",
    )

    # Set the path to the Gazebo world file
    world_file_path = LaunchConfiguration(
        "world_file_path",
        default=get_package_share_directory("my_robot_description")
    ...
    add_camera = Node(
        ...
        env={
            "GAZEBO_MODEL_PATH": get_package_share_directory("my_robot_description")
            + "/models"
        },
    )
